...
describe('Tutorial part 6', () => {
  ...
  describe('Heroes tests', () => {
    ...
    it(`adds back ${targetHero.name}`, async () => {
      ...
      element(by.css('input')).sendKeys(newName);
      ...
      expect(heroesAfter[numHeroes]).toEqual({ id: maxId + 1, name: newName });
    });
  ...
  describe('Progressive hero search', () => {
    ...
    const test = (description: string, numberOfSearchResults: number) => {
      it(`${description}`, async () => {
        getPageElts().searchBox.sendKeys('Ma');
        browser.sleep(1000);
        expect(getPageElts().searchResults.count()).toBe(numberOfSearchResults);
      });
    };

    test(`searches for 'Ma'`, 4);
    test(`continues search with 'g'`, 2);
